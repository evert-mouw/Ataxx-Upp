...
#include <ataxx/hulpfuncties.cpp>
#include <ataxx/bord.hpp>
#include <ataxx/stapel.hpp>
#include <ataxx/bord.cpp>
#include <ataxx/stapel.cpp>
//using namespace std;
...
void MyAppWindow::Paint(Draw& w) {
	...
	// GAAT FOUT ONDER LINUX
	if ( true==false && invoerstatus && bord->veld[invoer.xbron][invoer.ybron]==speler ) {
		...
		w.DrawEllipse(startx, starty, virusgrootte/2, virusgrootte/2, Yellow()); // GAAT FOUT ONDER LINUX
	}
	...
	if (bordwissel) {
		...
		//centrumpunt = GetWorkArea().CenterPoint(); // GAAT FOUT ONDER LINUX ????
		//centreerx = centrumpunt.x-(int)(.5*raambreedte);
		//centreery = centrumpunt.y-(int)(.5*raamhoogte);
		//SetRect(centreerx, centreery, raambreedte, raamhoogte); // GAAT FOUT ONDER LINUX
		bordwissel = false;
	}
}
