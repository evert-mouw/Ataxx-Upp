...
void ataxxbord::beginstand() {
	...
	veld[0][0] = 1;					// beginstand speler 1
	veld[maxkolom-1][maxrij-1] = 2;	// beginstand speler 2
}
...
bool ataxxbord::toegestanezet (zet test) {
	...
	if ( test.xdoel > maxrij-1 || test.ydoel > maxkolom-1 ) return false;
...
void ataxxbord::drukaf() {
	// toon het bord op het CLI / console scherm (niet GUI)

	int x,y;						// hulpvariabelen voor iteraties door veld

	cout << endl;

	// toont het bord als een rooster
	for (y=(maxrij-1);y>-1;y--) {
		cout << y+1 << " | ";
		for (x=0;x<maxkolom;x++) {
			cout << veld[x][y] << " ";
		}
	...
	cout << "  \\";
	for (x=0;x<maxkolom;x++) cout << "--";
	cout << endl;
	cout << "    ";
	for (x=0;x<maxkolom;x++) cout << x+1 << " ";

	// toont nog wat statistieken
	cout << endl;
	cout << "Speler 1 heeft " << telvirussen(1) << " virussen." << endl;
	cout << "Speler 2 heeft " << telvirussen(2) << " virussen." << endl;
	cout << endl;
}
...
zet ataxxbord::allezetten (int speler, int maxzet) {
	...
	// initialiseren mogenlijkezet; mocht er geen mogelijke zet
...
zet ataxxbord::strategiezet (int speler) {
	...
	do {
		...
		// test de zet in het tijdelijke bord ...
		doezet (kandidaatzet);
		// ... en bereken de score
		score = telvirussen(speler) - telvirussen(vijand);
		if ( score > maxscore ) {
			maxscore = score;
			maxzet = i;
		}
...
void ataxxbord::drukaf_stats() {
	// toon de stats voor de spelers
	if ( telvirussen(1) > telvirussen(2) )
		cout << "Speler 1 heeft gewonnen." << endl;
	if ( telvirussen(1) < telvirussen(2) )
		cout << "Speler 2 heeft gewonnen." << endl;
	if ( telvirussen(1) == telvirussen(2) )
		cout << "Gelijk spel." << endl;
	cout << "Speler 1 eindigd met " <<  telvirussen(1) << " virussen." << endl;
	cout << "Speler 2 eindigd met " <<  telvirussen(2) << " virussen." << endl;
}
